...
static int binary_matmul(struct matmul_data *data)
{
    ...
    enum gvml_vr16 vr_row_reuse = GVML_VR16_7;
    enum gvml_vr16 idx = GVML_VR16_8;
    enum gvml_vr16 result = GVML_VR16_3;
    enum gvml_vr16 Z1 = GVML_VR16_8;

    gvml_create_subgrp_index_u16(idx, GVML_P2_32K, GVML_P2_1K);

    gvml_cpy_imm_16(offset, 16);
    for (size_t out_row = 0; out_row < 1024; out_row++) { // 
        PROF_START(ld_lhs);
        if (out_row % 32 == 0) {
            direct_dma_l4_to_l1_32k(GVML_VM_0, in_I1); // index calculation left out
            gvml_load_16(vr_row_reuse, GVML_VM_0);
        }
        PROF_END(ld_lhs);
        gvml_duplicate_subgrp_16_grp_sgidx(vr_row, vr_row_reuse, idx, GVML_P2_8K, GVML_P2_1K, out_row % 32, GVML_VM_5);
