...
static int binary_matmul(struct matmul_data *data)
{
  ...
  for (size_t out_row = 0; out_row < 256; out_row++) {
    ...
    gvml_cpy_imm_subgrp_16_grp(
      inp_broadcast_vr,     /* enum gvml_vr16 vdst */
      GVML_P2_32K,          /* enum gvml_power2_sizes grp */
      GVML_P2_1K,          /* enum gvml_power2_sizes sgrp */
      inp_L4ptr + 1024 * out_row, /* const uint16_t *data */
      1 << 10);
    PROF_END(dma);
