...
static int run_bmatmul_cmd(gdl_context_handle_t ctx_id)
{
    ...
    uint64_t keys_size = 512 * 1024 * 1024 / 2; // 512 MB data
    uint16_t *keys = (uint16_t *)malloc(keys_size * sizeof(uint16_t));

    if (keys == NULL) {
        gsi_error("Failed to allocate memory for keys array\n");
        return -1;
    }
    ...
    const uint64_t keys_dev_buf_size = sizeof(uint16_t) * keys_size;
    const uint64_t output_dev_buf_size = sizeof(int16_t) * _32K;
    const uint64_t io_dev_buf_total_size = keys_dev_buf_size + output_dev_buf_size;
    ...
    ret = gdl_mem_cpy_to_dev(input_dev_bufs, keys, keys_dev_buf_size);
    if (ret)
    {
    ...
    struct program_cmd cmd = {
        .data.mem_hndl_keys = input_dev_bufs,
    };
    // Point output's L4 ptr to the place right after xy data
    ret = gdl_add_to_mem_handle(&cmd.data.mem_hndl_output, cmd.data.mem_hndl_keys, keys_dev_buf_size);
    if (ret)
    {
