...
static int my_kernel(struct program_data *data)
{
  ...
  for (size_t word_idx = 0; word_idx < 128; word_idx++)
  {
    ...
    gvml_cpy_imm_subgrp_16_grp(
      ...
      GVML_P2_32K,
      GVML_P2_8,
      words_L4ptr + word_idx * 32 * 1024 + 1,
      1 << 3
    );
    gvml_cpy_imm_subgrp_16_grp(
      word_vr2,
      GVML_P2_32K,
      GVML_P2_8,
      words_L4ptr + word_idx * 32 * 1024 + 2,
      1 << 3
    );
    ...
    for (size_t vmr = 0; vmr < 13; vmr++)
    {
      ...
      gvml_cpy_imm_16(GVML_VR16_1, 3);
      gvml_eq_16(eq_flag, tmp_vr, GVML_VR16_1); // compare with 3
      gvml_cpy_from_mrk_16_msk(tmp_vr, eq_flag, 0x0001);
