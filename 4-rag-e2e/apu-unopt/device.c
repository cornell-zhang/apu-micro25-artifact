  ...
  for (size_t tile = 0; tile < tile_num; tile++) {
    for (size_t dim_chunk = 0; dim_chunk < EMBED_DIM / 48; dim_chunk++) { // 48 is the number of VMRs
      ...
      for (uint16_t vmr = 0; vmr < 48; vmr++) {
        ...
        gvml_add_subgrps_u16_grp(data_magnitude_vr, data_magnitude_vr, GVML_P2_512, GVML_P2_1, 0, GVML_VM_47, tmp_vr);
        for(uint16_t q = 0; q < QUERY_COUNT; q++) { // query
          ...
          gvml_lookup_16(query_vr, GVML_VR16_0, query_L4ptr, 1);
          // calculate distance
          gvml_mul_s16(query_vr, query_vr, data_vr);
          gvml_add_subgrps_u16_grp(q, query_vr, GVML_P2_512, GVML_P2_1, 0, GVML_VM_47, tmp_vr);
        }
      }
...
GAL_TASK_ENTRY_POINT(apu_kernel_task, in, out)
{
  ...
  gsi_info("\nRunning KNN cosine distance! Opt3 200GB\n");
  gvml_init_once();
