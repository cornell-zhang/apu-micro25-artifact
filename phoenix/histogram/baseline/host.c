...
static int run_bmatmul_cmd(gdl_context_handle_t ctx_id)
{
    ...
    // for smaller arrays we can allocate themm in stack
    // if A and B are too big, we should allocate them in heap with malloc
    uint64_t input_size = 1024 * 1024 * 256 * 3; // remember this must be uint64_t
    uint16_t *A = (uint16_t *)malloc(input_size * sizeof(uint16_t));
    if (A == NULL) {
    ...
    const uint64_t input_dev_buf_size = sizeof(uint16_t) * input_size;
    const uint64_t output_dev_buf_size = sizeof(int16_t) * _32K;
    const uint64_t debug_dev_buf_size = sizeof(uint16_t) * _32K;
    const uint64_t io_dev_buf_total_size = input_dev_buf_size + output_dev_buf_size + debug_dev_buf_size;
    ...
    // Copy A's data to the start of the allocated memory
    ...
    // Point debug's L4 ptr to the place right after A's data
    ret = gdl_add_to_mem_handle(&cmd.data.mem_hndl_debug, cmd.data.mem_hndl_input, input_dev_buf_size);
    if (ret)
    {
    ...
    // Point output's L4 ptr to the place right after debug's data
    ret = gdl_add_to_mem_handle(&cmd.data.mem_hndl_output, cmd.data.mem_hndl_debug, debug_dev_buf_size);
    if (ret)
    {
        gsi_error("gdl_add_to_mem_handle() failed: %s\n", gsi_status_errorstr(ret));
        goto CLEAN_UP;
    ...
    // Copy the debug from L4 to host
    ...
    // int16_t golden_output[_32K];
    // binary matmul
    // for (size_t i = 0; i < _32K; i++) {
    //     golden_output[i] = A[i] + B[i];
    // }
