    ...
    for (size_t tile = 0; tile < total_data_size / tile_data_size; tile++) {
        ...
        for (size_t vmr_idx = 0; vmr_idx < 48; vmr_idx++) {
            for (size_t t = 0; t < num_transcations; t++) {
                trans[t].num_steps = 1;
                trans[t].step_size_num_512b_chunk = 32; // 32 chunks == 8K entries, one group.
                trans[t].l4_addr = (void *)(inp_L4ptr_moving + 8192 * vmr_idx); // each VMR contains 8K data, duplicated 4 times
            ...
            gal_fast_l2dma_mem_to_l2_start(GAL_L2DMA_APC_ID_1, 2, &trans[2],
                                            GAL_L2DMA_L2_READY_SET);
            direct_dma_l2_to_l1_32k(vmr_idx); // put them in the VMR
        }
        PROF_END(dma);
