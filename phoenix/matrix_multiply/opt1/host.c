...
static int run_bmatmul_cmd(gdl_context_handle_t ctx_id)
{
    ...
    const uint64_t input_dev_buf_size = sizeof(uint16_t) * DIM * DIM;
    const uint64_t weight_dev_buf_size = sizeof(uint16_t) * DIM * DIM;
    ...
    // change A's data layout from row-major to 32-blocked column major
    // uint16_t A_blocked_col[PACKED_MSIZE];
    // for (int out_group = 0; out_group < 32; out_group++) {
    //     for (int row = 0; row < 32; row++) {
    //         for (int col = 0; col < 64; col++) {
    //             int org_row = out_group * 32 + row;
    //             int org_col = col;
    //             int new_row = out_group * 64 + col;
    //             int new_col = row;
    //             A_blocked_col[new_row * 32 + new_col] = A[org_row * 64 + org_col];
    //         }
    //     }
    // }
