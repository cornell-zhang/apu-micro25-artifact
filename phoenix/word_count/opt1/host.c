...
static int run_bmatmul_cmd(gdl_context_handle_t ctx_id)
{
    ...
    uint64_t input_size = 10 * 1024 * 1024; // 10 MB data
    uint64_t unique_words = 3 * 32 * 1024; // 128 words, each word has 6 characters, so 6 bytes. 128 entries duplicated to entire VR. Totally 3 VR.
    uint16_t *input = (uint16_t *)malloc(input_size);
    uint16_t *words = (uint16_t *)malloc(unique_words * sizeof(uint16_t));

    if (input == NULL) {
        gsi_error("Failed to allocate memory for input array\n");
        return -1;
    }

    if (words == NULL) {
        gsi_error("Failed to allocate memory for words array\n");
        return -1;
    }

    int16_t output[_32K]; // one VR of group assignment

    for (uint32_t i = 0; i < input_size / sizeof(uint16_t); ++i){
        input[i] = rand() & 0xffff;
    }
    ...
    const uint64_t input_dev_buf_size = input_size;
    const uint64_t words_dev_buf_size = unique_words * sizeof(uint16_t);
    const uint64_t output_dev_buf_size = sizeof(int16_t) * _32K;
    const uint64_t io_dev_buf_total_size = input_dev_buf_size + words_dev_buf_size + output_dev_buf_size;
    ...
    // Copy input data to the start of the allocated memory
    ret = gdl_mem_cpy_to_dev(input_dev_bufs, input, input_dev_buf_size);
    if (ret)
    {
    ...
    struct program_cmd cmd = {
        .data.mem_hndl_input = input_dev_bufs,
    };
    ret = gdl_add_to_mem_handle(&cmd.data.mem_hndl_word, cmd.data.mem_hndl_input, input_dev_buf_size);
    if (ret)
    {
        gsi_error("gdl_add_to_mem_handle() failed: %s\n", gsi_status_errorstr(ret));
        goto CLEAN_UP;
    }
    ret = gdl_mem_cpy_to_dev(cmd.data.mem_hndl_word, words, words_dev_buf_size);
    if (ret)
    {
        gsi_error("gdl_mem_cpy_to_dev() failed: %s\n", gsi_status_errorstr(ret));
        goto CLEAN_UP;
    }
    ret = gdl_add_to_mem_handle(&cmd.data.mem_hndl_output, cmd.data.mem_hndl_word, words_dev_buf_size);
    if (ret)
    {
        gsi_error("gdl_add_to_mem_handle() failed: %s\n", gsi_status_errorstr(ret));
        goto CLEAN_UP;
